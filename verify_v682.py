...
def test_tool(tool_name, args={}):
    ...
    try:
        # Use a dummy API key if needed, though local might not require it depending on config
        ...
        stream_resp = requests.get(f"http://localhost:8000/stream/{call_id}", stream=True, headers=headers)
...
if __name__ == "__main__":
    check_version()
    test_tool("system_cpu_user_time_avg_audit")
    test_tool("system_cpu_system_time_avg_audit")
    test_tool("system_cpu_idle_time_avg_audit")
