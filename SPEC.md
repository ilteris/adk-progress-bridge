...
*   **REST Flow (SSE):**
    ...
    *   `GET /health`: Returns system health status, version, git commit, uptime, CPU count, thread count, load average, active task count, total tasks started, memory usage, and configuration parameters (heartbeat timeout, cleanup interval, etc.).
