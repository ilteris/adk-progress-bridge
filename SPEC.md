...
*   **REST Flow (SSE):**
    ...
    *   `GET /health`: Returns system health status, version (**1.4.9**), git commit, uptime, CPU count, thread count, active WebSocket connections, **WebSocket messages received/sent counters**, load average, active task count, total tasks started, memory usage, CPU usage, **Open File Descriptors count**, **Thread count**, **Network Throughput (Bytes/sec)**, **Context Switches (Voluntary/Involuntary)**, **Task Success Rate %**, and configuration parameters (heartbeat timeout, cleanup interval, max concurrent tasks, etc.). and `adk_build_info` metric. Includes comprehensive system metrics: Disk usage, Swap memory, Page faults (**including real-time rates**), Network packets, CPU frequency, Disk I/O, and **Process-specific IO counters including real-time throughput**.
    *   `GET /version`: Returns current API version (**1.4.9**), git commit hash, and operational status (e.g., "THE TRANSCENDENCE").
...
7.  **Operational Visibility:** Enhanced health monitoring with real-time tracking of active WebSocket connections, total message throughput (**Bytes/sec**), and resource utilization (CPU, Memory, FDs, Threads, **Context Switches**, **Page Faults**).
...
9.  **THE TRANSCENDENCE Observability:** Version 1.4.9 (v359) reaches the ultimate operational apex, adding real-time page fault rates and WebSocket frame integrity tracking. Includes all previous tiers: Apotheosis, Nirvana, Beyond Singularity, Ascension Singularity, Transcendence, Omnipotence, Deification, THE OVERLORD, and THE ETERNITY.