...
def test_ws_v671_system_cpu_stats_syscalls_focused_audit():
    client = TestClient(app)
    with client.websocket_connect("/ws?api_key=test-key") as websocket:
        # Initial connected message
...
def test_ws_v671_system_net_io_packets_sent_focused_audit():
    client = TestClient(app)
    with client.websocket_connect("/ws?api_key=test-key") as websocket:
        # Initial connected message
        websocket.receive_json()
...
def test_ws_v671_system_net_io_packets_recv_focused_audit():
    client = TestClient(app)
    with client.websocket_connect("/ws?api_key=test-key") as websocket:
        # Initial connected message
        websocket.receive_json()
