...
def test_v585_metadata():
    client = TestClient(app)
    response = client.get("/version")
    assert response.status_code == 200
    data = response.json()
    assert data["version"] == "2.1.1"
    assert data["status"] == "v585 SUPREME APEX VERIFICATION ADELE"
    assert data["git_commit"] == "v585-supreme-apex-adele-verification"

@pytest.mark.asyncio
async def test_v585_ws_health_metrics():
    ...
    with TestClient(app).websocket_connect(f"/ws?api_key={api_key}") as websocket:
        ...
        # Get Health
        websocket.send_json({"type": "get_health", "request_id": "v585-test"})
        resp = websocket.receive_json()
        assert resp["type"] == "health_data"
        assert resp["request_id"] == "v585-test"
        assert resp["data"]["version"] == "2.1.1"
        assert resp["data"]["operational_apex"] == "v585 SUPREME APEX VERIFICATION ADELE"

def test_v585_health_endpoint():
    client = TestClient(app)
