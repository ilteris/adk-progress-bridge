...
def test_auth_disabled_by_default(monkeypatch):
    # Explicitly disable auth for this test
    monkeypatch.setattr(auth, "BRIDGE_API_KEY", None)
    # When BRIDGE_API_KEY is not set, it should allow access
    response = client.post("/start_task/long_audit", json={"args": {"duration": 1}})
...
def test_auth_enabled_wrong_key(enable_auth):
    response = client.post("/start_task/long_audit", json={"args": {"duration": 1}}, headers={"X-API-Key": "wrong-key"})
    assert response.status_code == 401

def test_auth_enabled_correct_key(enable_auth):
    response = client.post("/start_task/long_audit", json={"args": {"duration": 1}}, headers={"X-API-Key": "test-secret-key"})
    assert response.status_code == 200
