...
def test_v601_metadata_verification():
    """
    Verifies that the version and metadata are correctly updated for v601.
    """
    with TestClient(app) as client:
        ...
        assert "v601" in data["git_commit"]
        assert "v601" in data["status"]
