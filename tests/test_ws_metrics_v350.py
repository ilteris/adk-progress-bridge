...
def test_health_v350_apotheosis():
    ...
    assert data["operational_apex"] in ["APOTHEOSIS", "ULTIMA", "OMNIPRESENCE", "THE SOURCE", "THE ONE", "THE SINGULARITY", "THE OMEGA"]
...
def test_version_v350():
    ...
    data = response.json()
    assert data["version"] >= "1.4.0"
    assert data["status"] in ["APOTHEOSIS", "ULTIMA", "OMNIPRESENCE", "THE SOURCE", "THE ONE", "THE SINGULARITY", "THE OMEGA"]