...
def test_process_memory_percent_audit_tool_ws():
    """
    Verifies that the process_memory_percent_audit tool (v655) works over WebSocket.
    """
...
def test_v655_metadata_baseline():
    """
    Verifies the metadata baseline for v655.
    Note: We've moved to v655 in the current code, but we check if the baseline tools are present.
    """
    client = TestClient(app)
    response = client.get("/version")
    assert response.status_code == 200
    data = response.json()
    # If we are in v655 session, version will be 2.6.4
