...
def test_process_memory_percent_audit_tool_ws():
    """
    Verifies that the process_memory_percent_audit tool (v663) works over WebSocket.
    """
...
def test_v663_metadata_baseline():
    """
    Verifies the metadata baseline for v663.
    Note: We've moved to v663 in the current code, but we check if the baseline tools are present.
    """
    client = TestClient(app)
    response = client.get("/version")
    assert response.status_code == 200
    data = response.json()
    # If we are in v663 session, version will be 2.6.4
