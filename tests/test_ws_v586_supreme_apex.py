...
def test_v608_metadata():
    ...
    assert data["status"] == "v608 SUPREME APEX VERIFICATION ADELE"
    assert data["git_commit"] == "v608-supreme-apex-adele-verification"

@pytest.mark.asyncio
async def test_v608_ws_health_metrics():
    ...
    with TestClient(app).websocket_connect(f"/ws?api_key={api_key}") as websocket:
        ...
        # Get Health
        websocket.send_json({"type": "get_health", "request_id": "v608-test"})
        resp = websocket.receive_json()
        assert resp["type"] == "health_data"
        assert resp["request_id"] == "v608-test"
        assert resp["data"]["version"] == "2.3.4"
        assert resp["data"]["operational_apex"] == "v608 SUPREME APEX VERIFICATION ADELE"

@pytest.mark.asyncio
async def test_v608_resource_monitor_tool():
    from backend.app.auth import BRIDGE_API_KEY
    api_key = BRIDGE_API_KEY or "test-key"
