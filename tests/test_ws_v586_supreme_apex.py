...
def test_v674_metadata():
    ...
    assert data["status"] == "v674 SUPREME APEX VERIFICATION ADELE"
    assert data["git_commit"] == "v674-supreme-apex-adele-verification"

@pytest.mark.asyncio
async def test_v674_ws_health_metrics():
    ...
    with TestClient(app).websocket_connect(f"/ws?api_key={api_key}") as websocket:
        ...
        # Get Health
        websocket.send_json({"type": "get_health", "request_id": "v674-test"})
        resp = websocket.receive_json()
        assert resp["type"] == "health_data"
        assert resp["request_id"] == "v674-test"
        assert resp["data"]["version"] == "2.10.0"
        assert resp["data"]["operational_apex"] == "v674 SUPREME APEX VERIFICATION ADELE"

@pytest.mark.asyncio
async def test_v674_resource_monitor_tool():
    from backend.app.auth import BRIDGE_API_KEY
    api_key = BRIDGE_API_KEY or "test-key"
