...
def test_v596_metadata():
    client = TestClient(app)
    response = client.get("/version")
    assert response.status_code == 200
    data = response.json()
    assert data["version"] == "2.2.2"
    assert data["status"] == "v596 SUPREME APEX VERIFICATION ADELE"
    assert data["git_commit"] == "v596-supreme-apex-adele-verification"

@pytest.mark.asyncio
async def test_v596_ws_health_metrics():
    ...
    with TestClient(app).websocket_connect(f"/ws?api_key={api_key}") as websocket:
        ...
        # Get Health
        websocket.send_json({"type": "get_health", "request_id": "v596-test"})
        resp = websocket.receive_json()
        assert resp["type"] == "health_data"
        assert resp["request_id"] == "v596-test"
        assert resp["data"]["version"] == "2.2.2"
        assert resp["data"]["operational_apex"] == "v596 SUPREME APEX VERIFICATION ADELE"

@pytest.mark.asyncio
async def test_v596_resource_monitor_tool():
    from backend.app.auth import BRIDGE_API_KEY
    api_key = BRIDGE_API_KEY or "test-key"
