...
def test_v664_metadata():
    ...
    assert data["status"] == "v664 SUPREME APEX VERIFICATION ADELE"
    assert data["git_commit"] == "v664-supreme-apex-adele-verification"

@pytest.mark.asyncio
async def test_v664_ws_health_metrics():
    ...
    with TestClient(app).websocket_connect(f"/ws?api_key={api_key}") as websocket:
        ...
        # Get Health
        websocket.send_json({"type": "get_health", "request_id": "v664-test"})
        resp = websocket.receive_json()
        assert resp["type"] == "health_data"
        assert resp["request_id"] == "v664-test"
        assert resp["data"]["version"] == "2.9.0"
        assert resp["data"]["operational_apex"] == "v664 SUPREME APEX VERIFICATION ADELE"

@pytest.mark.asyncio
async def test_v664_resource_monitor_tool():
    from backend.app.auth import BRIDGE_API_KEY
    api_key = BRIDGE_API_KEY or "test-key"
