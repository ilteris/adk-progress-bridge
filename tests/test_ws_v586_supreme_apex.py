...
def test_v616_metadata():
    ...
    assert data["status"] == "v618 SUPREME APEX VERIFICATION ADELE"
    assert data["git_commit"] == "v617-supreme-apex-adele-verification"
...
async def test_v616_ws_health_metrics():
    ...
    with TestClient(app).websocket_connect(f"/ws?api_key={api_key}") as websocket:
        ...
        assert resp["data"]["version"] == "2.4.4"
        assert resp["data"]["operational_apex"] == "v618 SUPREME APEX VERIFICATION ADELE"
