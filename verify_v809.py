...
async def verify_v809():
    ...
    try:
        async with websockets.connect(url) as websocket:
            ...
            await websocket.send(json.dumps({"type": "list_tools", "request_id": "v809-list"}))
            ...
            if len(tools) >= 1320:
                print(f"SUCCESS: {len(tools)} tools reached milestone (1320+).")
            else:
                print(f"FAILURE: Only {len(tools)} tools found.")

            # 2. Test one of the new V5 tools
            test_tool = "system_disk_io_read_bytes_sum_v5"
            print(f"Testing tool: {test_tool}")
            await websocket.send(json.dumps({
                "type": "start",
                "tool_name": test_tool,
                "request_id": "v809-test-tool"
            }))
